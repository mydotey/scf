...
impl DefaultRawPropertyConfig {
    pub fn check_valid(&self) {
        ...
        if new_value.is_none() {
            panic!(
                "None is returned when the value filter filters the default value, \
...
impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> PartialEq
...
impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> fmt::Debug
    for DefaultPropertyConfig<K, V>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            self.get_raw_key(),
            self.get_value_type(),
            self.get_raw_default_value(),
            self.get_value_converters(),
            self.get_value_filter().type_name(),
            self.get_doc(),
            self.is_static(),
            self.is_required()
        )
...
unsafe impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> Sync
    for DefaultPropertyConfig<K, V>
{
}
unsafe impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> Send
    for DefaultPropertyConfig<K, V>
{
}
...
impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> PropertyConfig<K, V>
    for DefaultPropertyConfig<K, V>
{
    ...
    fn get_default_value(&self) -> Option<Box<V>> {
        self.raw
            .get_raw_default_value()
            .map(|v| Box::new(v.as_ref().as_any_ref().downcast_ref::<V>().unwrap().clone()))
    }
    ...
    fn build(&self) -> Box<dyn PropertyConfig<K, V>> {
        let raw = DefaultRawPropertyConfig {
            ...
            default_value: self
                ...
                .as_ref()
                .map(|v| v.clone())
                .map(|v| ImmutableValue::wrap(v)),
            value_converters: Arc::new(self.value_converters.clone()),
            value_filter: self
                .value_filter
                .as_ref()
                .map(|f| RawValueFilter::clone_boxed(f.as_ref())),
            doc: self.doc.clone(),
...
impl DefaultRawProperty {
    ...
    pub fn update(
        ...
        source: Option<Box<dyn ConfigurationSource>>,
    ) {
...
impl fmt::Debug for DefaultRawProperty {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            self.get_raw_value(),
            self.get_source()
                .map(|v| v.get_config().get_name().to_string())
        )
    }
}
...
impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> DefaultProperty<K, V> {
    pub fn from_raw(property: &dyn RawProperty) -> Self {
        DefaultProperty {
            config: Arc::new(Box::new(DefaultPropertyConfig::from_raw(
...
impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> Property<K, V>
    for DefaultProperty<K, V>
{
    ...
    fn add_change_listener(&self, listener: PropertyChangeListener<K, V>) {
        self.raw
            .add_raw_change_listener(Arc::new(Box::new(move |e| {
...
impl PartialEq for DefaultRawPropertyChangeEvent {
    fn eq(&self, other: &Self) -> bool {
        self.property.as_ref() == other.property.as_ref()
...
impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> Eq
    for DefaultPropertyChangeEvent<K, V>
{
}
...
impl<K: ?Sized + KeyConstraint, V: ?Sized + ValueConstraint> RawPropertyChangeEvent
    for DefaultPropertyChangeEvent<K, V>
{
    fn get_raw_property(&self) -> &dyn RawProperty {
...
mod test {
    ...
    fn property_config_test() {
        ...
        assert_eq!(
            Ok(Value::to_boxed(1)),
            rc.convert_raw(Value::as_trait_ref(&s))
        );
        assert_ne!(
            Ok(Value::to_boxed(2)),
            rc.convert_raw(Value::as_trait_ref(&s))
        ...
        assert_eq!(
            ...
            rf.filter_raw(Value::to_boxed(11))
        );
        ...
        assert!(
            !config
                .as_ref()
                .reference_equals(config2.as_ref().as_any_ref())
        );
        assert!(config.as_ref().equals(config2.as_ref().as_any_ref()));
        assert!(
            RawPropertyConfig::as_trait_ref(config.as_ref())
                .equals(RawPropertyConfig::as_trait_ref(config2.as_ref()).as_any_ref())
        );
        assert_eq!(
            &RawPropertyConfig::clone_boxed(config.as_ref()),
    ...
    fn property_test() {
        ...
        assert!(
            property
                .get_raw_config()
                .equals(config.as_ref().as_any_ref())
        );
        ...
        property.add_raw_change_listener(Arc::new(Box::new(move |e| {
            println!(
                "property: {:?}, old_value: {:?}, new_value: {:?}, change_time: {:?}",
                e.get_raw_property(),
                e.get_raw_old_value(),
                e.get_raw_new_value(),
                e.get_change_time()
        ...
        let event = DefaultRawPropertyChangeEvent::new(
            arc_property,
            Some(ImmutableValue::new(0)),
            Some(ImmutableValue::new(0)),
    ...
    fn property_change_event_test() {
        let config = DefaultPropertyConfigBuilder::new()
            .set_key(Box::new(1))
            .set_default_value(Box::new(2))
            .build();
        let property = DefaultRawProperty::new(RawPropertyConfig::as_trait_ref(config.as_ref()));
